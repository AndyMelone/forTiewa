...
class EventCategoryDeleteView(LoginRequiredMixin, DeleteView):
    ...
    success_url = reverse_lazy('admin_events:event-category-list')
...
def create_event(request):
    ...
    if request.method == 'POST':
        ...
        if event_form.is_valid() and event_image_form.is_valid() and event_agenda_form.is_valid():
            event.created_user_id = request.user.id
            ef = event_form.save()
            ...
            return redirect('admin_events:event-list')
    context = {
        'form': event_form,
...
class EventCreateView(LoginRequiredMixin, CreateView):
    ...
    success_url = reverse_lazy('admin_events:event-list')

    def form_valid(self, form):
        evt = form['event'].save(commit=False)
        
        evt.created_user_id = self.request.user.id  # Set the created_user_id
        evt.save()
...
class EventUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.created_user_id = self.request.user.id  # Set the created_user_id
        return super().form_valid(form)
...
class RemoveEventMemberDeleteView(LoginRequiredMixin, DeleteView):
    ...
    success_url = reverse_lazy('admin_events:join-event-list')
...
class RemoveEventUserWishDeleteView(LoginRequiredMixin, DeleteView):
    ...
    success_url = reverse_lazy('admin_events:event-wish-list')
...
class AddUser(LoginRequiredMixin, CreateView):
    ...
    template_name = 'events/add_user.html'
    success_url = reverse_lazy('admin_events:list_user')
    ...
    def add_user(request):
        if request.method == 'POST':
            ...
            if form.is_valid():
                form.save()
                return redirect('admin_events:list_user')
        else:
